...
- **背单词（翻卡复习）**：汇总全部课程单词（按词去重），支持按级别筛选；翻卡看图、
  「认识 / 不认识」标记，不认识的词稍后再次出现，已掌握的词本地记忆。
- **真人发音 · 美式 / 英式**：背单词页用**有道词典真人发音**，可一键切换美式 / 英式（需联网）；
  联网失败时按所选口音回退浏览器语音。
- **课程发音**：本地用 macOS `Samantha` 语音批量生成的音频文件（`audio/*.m4a`），
...
> **背单词真人发音依赖外部域名 `dict.youdao.com`**。部署到带严格 CSP 的环境（如 Coze webview）时，
> 需在内容安全策略里放行 `media-src https://dict.youdao.com`（或 `connect-src`），否则美式/英式发音
> 会回退到浏览器语音。课程内题型用的是本地 `audio/*.m4a`，不受影响。
